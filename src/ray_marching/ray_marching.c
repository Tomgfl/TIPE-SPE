...
//     vector Lumiere = {2.5, 0, 0}; // lumiere mis a la mano
...
//             float val_light = all_light(position_actuelle, Lumiere, scene);
//             // clock_t end_l = clock();
...
void* ray_marching_bvh_thread(void* ptr_args){
    ...
    for (int i = a->id*WIDTH/NB_THREADS; i < (a->id+1)*WIDTH/NB_THREADS; i++){
        for (int j = 0; j < HEIGHT; j++){
            a->t_out[i][j] = ray_marching_bvhNoPlan(a->t_in[i][j], a->bvh, a->fct_scene_p, a->fct_scene_bvh_mov);
        }
    }
    return NULL;
}
...
void* ray_marching_bvh_thread_move(void* ptr_args){
    arg* a = (arg*) ptr_args;
    for (int i = a->id*WIDTH/NB_THREADS; i < (a->id+1)*WIDTH/NB_THREADS; i++){
        for (int j = 0; j < HEIGHT; j++){
            a->t_out[i][j] = ray_marching_bvh_moving(a->t_in[i][j], a->bvh, a->fct_scene_p, a->fct_scene_bvh_mov);
        }
    }
    return NULL;
}
...
color ray_marching_bvh(ray r, BVHNode* scene, res_SDF (*scene_actuelle)(vector), res_SDF (*scene_bvh)(BVHNode*, vector, res_SDF), vector n, vector m, color c){
    ...
    for (int i = 0; i < MAX_RAY_STEPS; i++){
        ...
        if (dist < DIST_MIN){ // Si on touche un objet
            ...
            // float val_light = 1;
            // clock_t begin_s = clock();
            float val_shadow = shadow_2(position_actuelle, Lumiere, 32, scene);
            // float val_shadow = 1.0;
            // clock_t end_s = clock();
            // STATS.temps_shadow += (double)(end_s - begin_s)/CLOCKS_PER_SEC;
            // color res = {150.0, 150.0, 15.0, val_light*val_shadow};
...
color ray_marching_bvhNoPlan(ray r, BVHNode* scene, res_SDF (*scene_actuelle)(vector), res_SDF (*scene_bvh)(BVHNode*, vector)){
    ...
    vector Lumiere = {5, 20, -0.5}; // lumiere mis a la mano
...
color ray_marching_bvh_moving(ray r, BVHNode* scene, res_SDF (*scene_actuelle)(vector), res_SDF (*scene_bvh)(BVHNode*, vector)){
    ...
    vector Lumiere = {-10, 10, -1}; // lumiere mis a la mano
...
color ray_marching_editor(ray r, BVHNode* scene_root){
    ...
    for (int i = 0; i < MAX_RAY_STEPS; i++){
        ...
        res_SDF result_scene = traverseBVH(scene_root, position_actuelle, max_dist_init);
        float dist = result_scene.dist;
        dist_tot += dist;

        if (dist < DIST_MIN){  // Si on touche un objet
            float eps = 0.01;
            float dx = traverseBVH(scene_root, (vector){position_actuelle.x+eps, position_actuelle.y, position_actuelle.z}, max_dist_init).dist - dist;
            float dy = traverseBVH(scene_root, (vector){position_actuelle.x, position_actuelle.y+eps, position_actuelle.z}, max_dist_init).dist - dist;
            float dz = traverseBVH(scene_root, (vector){position_actuelle.x, position_actuelle.y, position_actuelle.z+eps}, max_dist_init).dist - dist;
            vector norm = normalise_vecteur((vector){dx, dy, dz});

            vector dir_lum = normalise_vecteur(get_vec_2_pts(position_actuelle, Lumiere));
            float diff = fmax(prod_scal(norm, dir_lum), 0.2); 
            
            color res = result_scene.c;
            res.r *= diff; res.g *= diff; res.b *= diff;
            return res;
        }

        if (dist_tot > MAX_TOTAL_LENGHT) return c_fond; 
