...
// Cette fonction détruit l'arbre MAIS PAS les objets
void freeBVH_Editor(BVHNode* node) {
    if (node == NULL) return;

    // Adapte les noms si ta struct BVHNode est différente
    if (node->left) freeBVH_Editor(node->left);
    if (node->right) freeBVH_Editor(node->right);

    // On libère le noeud lui-même, mais PAS node->obj
    free(node);
}
