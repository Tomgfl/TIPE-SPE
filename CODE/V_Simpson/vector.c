...
res_SDF maxsdf = (res_SDF){FLT_MAX, (color){0,0,255,1.0}};




// renvoie le vecteur normal a une surface (en un point proche d'une surface) a l'aide du gradient
vector vect_normal(vector pts, BVHNode* scene){
    float dist = traverseBVH(scene, pts, maxsdf).dist;
    ...
    float grad_x = traverseBVH(scene, (v_add(pts, (vector){EPSILON,0,0})), maxsdf).dist - dist;
    float grad_y = traverseBVH(scene, (v_add(pts, (vector){0,EPSILON,0})), maxsdf).dist - dist;
    float grad_z = traverseBVH(scene, (v_add(pts, (vector){0,0,EPSILON})), maxsdf).dist - dist;
